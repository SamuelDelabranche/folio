...
// ── F tube pur Flutter ────────────────────────────────────
class _TubeLetter extends StatelessWidget {
  const _TubeLetter();

  @override
  Widget build(BuildContext context) {
    return CustomPaint(
      size: const Size(130, 170),
      painter: _FTubePainter(),
    );
  }
}

class _FTubePainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final w = size.width;
    final h = size.height;

    // Dégradé global rose→violet
    final grad = LinearGradient(
      begin: Alignment.bottomLeft,
      end: Alignment.topRight,
      colors: const [Color(0xFFE91E8C), Color(0xFF9B5DE5)],
    );
    ...
    final paths = [
      // Trait vertical
      _segment(Offset(r, h - r), Offset(r, r), thick),
      // Barre haute
      _segment(Offset(r, r), Offset(w - r * 0.6, r), thick),
      // Barre milieu
      _segment(Offset(r, h * 0.48), Offset(w * 0.75, h * 0.48), thick),
    ];
  ...
  Path _segment(Offset a, Offset b, double _) {
    return Path()
      ..moveTo(a.dx, a.dy)
      ..lineTo(b.dx, b.dy);
  }
  ...
  @override
  bool shouldRepaint(_FTubePainter old) => false;
}
